...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			onSelectRow: null,
            onCheckRow: null,   //[sshi 10/13/2011]
			onSortCol: null,
			ondblClickRow: null,
			...
			scroll: false,
			multiboxonly : false,
            checkMultiboxWhenSelect: true,  //[sshi 10/10/2011]
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					...
					$('#sp_1',pgboxes).html($.fmatter ? $.fmatter.util.NumberFormat(ts.p.lastpage,fmt):ts.p.lastpage+moreIndicator);
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
					    ...
					    var moreIndicator = "";
				        if(ts.p.lastpage>=10000) {
				            moreIndicator = "+";
				        }else {
				            moreIndicator = "";
				    }
						$(".ui-paging-info",pgboxes).html($.jgrid.format(ts.p.recordtext,from,to,tot)+moreIndicator);
					}
				}
					...
					$(ts.rows).each(function(i) {
						if ( i>0 ) {
							if(!$(this).hasClass("subgrid") && !$(this).hasClass("jqgroup")){
                                if(ts.p.checkMultiboxWhenSelect) {
			...
			if(cSel === true) {
				if(ts.p.cellEdit === true) {
					...
					if(ts.p.multiselect && ts.p.multiboxonly) {
						if(scb){$(ts).jqGrid("setSelection",ptr[0].id,true, scb);}
						else {
                            if(ts.p.checkMultiboxWhenSelect){
							...
							$(ts).jqGrid("setSelection",ptr[0].id,true, scb);
						}
					} else {
						$(ts).jqGrid("setSelection",ptr[0].id,true, scb);
					}
                ...
                if(scb && $.isFunction(ts.p.onCheckRow)) {
                    var checked = $(td).find('input.cbox').attr('checked');
                    ts.p.onCheckRow.call(ts, ptr[0].id, checked == true ? true : false);
                }
				e.stopPropagation();
			} else {
				return this;
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr,fireInCheckboxCell) {
		return this.each(function(){
			...
			if(!$t.p.multiselect) {
				...
				if (  ia === -1 ){
				    //TODO: need further research :(
				    $($t.rows).removeClass("ui-state-highlight").attr("aria-selected","false");
					if(pt.className !== "ui-subgrid" && !fireInCheckboxCell) { $(pt).addClass("ui-state-highlight").attr("aria-selected","true");}
					stat = true;
                    
                    if($t.p.checkMultiboxWhenSelect || fireInCheckboxCell){
					    $("#jqg_"+$t.p.id+"_"+$.jgrid.jqID($t.p.selrow)).attr("checked",stat);
					    $t.p.selarrrow.push($t.p.selrow);
                    }

					if( $t.p.onSelectRow && onsr && !fireInCheckboxCell) { $t.p.onSelectRow.call($t,$t.p.selrow, stat); }
				} else {
					if(pt.className !== "ui-subgrid" && $t.p.checkMultiboxWhenSelect) { 
                    ...
                    if($(pt).find('input.cbox').attr('checked')) {
                        $(pt).addClass("ui-state-highlight").attr("aria-selected","true");
                    }
       ...
       setChecked : function(selection,onsr) {
		return this.each(function(){
			var $t = this, stat,pt, ner, ia, tpsr;
			if(selection === undefined) { return; }
			onsr = onsr === false ? false : true;
			...
			if(!$t.p.multiselect) {
				if(pt.className !== "ui-subgrid") {
					...
					$t.p.selrow = pt.id;
					$(pt).addClass("ui-state-highlight").attr("aria-selected","false");
					if( $t.p.onCheckRow && onsr) { $t.p.onCheckRow.call($t,$t.p.selrow, true); }
				}
			} else {
				$t.p.selrow = pt.id;
				ia = $.inArray($t.p.selrow,$t.p.selarrrow);
				if (  ia === -1 ){ 
					...
					$("#jqg_"+$t.p.id+"_"+$.jgrid.jqID($t.p.selrow)).attr("checked",stat);
					$t.p.selarrrow.push($t.p.selrow);
					if( $t.p.onCheckRow && onsr) { $t.p.onCheckRow.call($t,$t.p.selrow, stat); }
				} else {
