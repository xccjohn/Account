	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			var triggerToolbar = function() {
				var sdata={}, j=0, v, nm, sopt={}, filterCriterias = {};
				$.each($t.p.colModel,function(i,n){
					...
					switch (this.stype) {
						...
						case 'text':
							...
							if(v) {
                                filterCriterias[nm] = v;
                                if(v.indexOf('%')==0 && !this.searchoptions || ( this.searchoptions && !this.searchoptions.sopt)){
                                    sdata[nm] = v.substr(1);
								    sopt[nm] = 'cn';
                                }else{
                                    sdata[nm] = v;
							...
							break;
                        case 'daterange':
							v1 = $("input[name$="+nm+"From]",$t.grid.hDiv).val();
							v2 = $("input[name$="+nm+"To]",$t.grid.hDiv).val();
							if(v1) {
                                if(v1.indexOf('%')==0 && !this.searchoptions || ( this.searchoptions && !this.searchoptions.sopt)){
                                    sdata[nm+"From"] = v1;
								    sopt[nm+"From"] = 'cn';
                                }else{
                                    sdata[nm+"From"] = v1;
								...
								try {
									delete $t.p.postData[nm+"From"];
								} catch (e) {}
							}
                            if(v2) {
                                if(v2.indexOf('%')==0 && !this.searchoptions || ( this.searchoptions && !this.searchoptions.sopt)){
                                    sdata[nm+"To"] = v2;
								    sopt[nm+"To"] = 'cn';
                                }else{
                                    sdata[nm+"To"] = v2;
								...
								try {
									delete $t.p.postData[nm+"To"];
								} catch (e) {}
							}
							break;    
                        //add:john.xie
                        //date:2011-05-25
                        //desc:for user defined dropdown control
                        case 'ibselect':
                            v = $("select[name$="+nm+"]",$t.grid.hDiv).next().find('input[type=text]').val();
                            vkey = $("select[name$="+nm+"]",$t.grid.hDiv).attr("selectedValue");
                            if(n.formatter==='checkbox' && n.searchoptions!=undefined && !n.searchoptions.isSearch){
                                sdata[nm] = '';
                                sdata[nm+"Key"]='';
								sopt[nm] = so;
								j++;
                            }else if(n.searchoptions!=undefined && n.searchoptions.likeAll!=undefined && n.searchoptions.likeAll==v){
                                sdata[nm] = '';
                                sdata[nm+"Key"]='';
								sopt[nm] = so;
								j++;
                            }
                            else if(v&&v!=$("select[name$="+nm+"]",$t.grid.hDiv).ibselect("option","noneItemText")){
                                if(n.searchoptions && n.searchoptions.trueVal){
                                    //sdata[nm] = n.searchoptions.trueVal[''+v+''];
                                    sdata[nm] = n.searchoptions.trueVal['val'][$.inArray(v,n.searchoptions.trueVal['property'])];
                                    sdata[nm+"Key"]=vkey;
								    sopt[nm] = so; 
                                }else{
								    sdata[nm] = v;
								...
								try {
									delete $t.p.postData[nm];
                                    delete $t.p.postData[nm+"Key"];

								} catch (e) {}
							}
                            filterCriterias[nm] = sdata[nm];
                        break;
					}
				});
				var sd =  j>0 ? true : false;
				if(p.stringResult === true || $t.p.datatype == "local"|| $.isFunction($t.p.datatype)) {
					...
					$.each(sdata,function(i,n){
						...
						ruleGroup += "{\"field\":\"" + i + "\",";
						ruleGroup += "\"op\":\"" + sopt[i] + "\",";
						ruleGroup += "\"data\":\"" + (typeof n == 'string' ? n.replace('\"','\\"') : n) + "\"}";
						gi++;
					});
					ruleGroup += "]}";
				...
				var bsr = false;
				if($.isFunction(p.beforeSearch)){bsr = p.beforeSearch.call($t,filterCriterias);}
			...
			$.each($t.p.colModel,function(i,n){
				...
				th = $("<th role='columnheader' class='ui-widget-content ui-th-column ui-th-"+$t.p.direction+"'></th>");
				thd = $("<div style='width:100%;position:relative;height:100%;padding-right:0.3em;'></div>");
				if(this.hidden===true) { $(th).css("display","none");}
				this.search = this.search === false ? false : true;
				if(typeof this.stype == 'undefined' ) {this.stype='text';}
				soptions = $.extend({},this.searchoptions || {});
                var nameSeed = Math.round(Math.random()*9000)+1000;
				if(this.search){
					switch (this.stype)
					{
					case "select":
					...
					case 'text':
						...
						$(thd).append("<input type='text' style='width:90%;padding-top:2px;' name='"+nameSeed+(cm.index || cm.name)+"' id='gs_"+cm.name+"' value='"+df+"'/>");
                      ...
                      case 'daterange':
                        var df = soptions.defaultValue ? soptions.defaultValue: "";
                          var dateFromName = nameSeed + (cm.index || cm.name) + "From";
                          var dateToName = nameSeed + (cm.index || cm.name) + "To";
                          
                        var dateFrom = $("<input name='"+dateFromName+"' id='gs_"+dateFromName+"' value='"+df+"'/>").css({ "height": "14px", "width": "45%" }); ;
                            var dateTo = $("<input name='"+dateToName+"' id='gs_"+dateToName+"'/>").css({ "height": "14px", "width": "45%" }); ;
                             $(thd).append(dateFrom).append("<label>:</label>").append(dateTo);
                             $("input[name='"+dateFromName+"'],input[name='"+dateToName+"']",thd).datepicker({
                                //defaultDate: "+1w",
                                dateFormat:$.datepicker.regional[$.global.culture.name].dateFormat,
                                changeMonth: true,
                                onSelect: function (selectedDate) {
                                    var option = this.name.indexOf("From") >= 0 ? "minDate" : "maxDate",
                                    instance = $(this).data("datepicker"),
                                    date = $.datepicker.parseDate(
                                    instance.settings.dateFormat ||
                                    $.datepicker._defaults.dateFormat,
                                    selectedDate, instance.settings);
                                    $(thd).find("input").not(this).datepicker ("option", option, date);
                                    triggerToolbar();
                                }
                              });
                              $(thd).find("input").datepicker('option', $.datepicker.regional[$.global.culture.name]);
                              if(p.autosearch===true){
							        if(p.searchOnEnter) {
								        $("input",thd).keypress(function(e){
                      ...
                      case 'ibselect':
                                ...
                                //4:event
                                //5:init
                                $(thd).append("<select name='"+nameSeed+(cm.index || cm.name)+"' id='gs_"+cm.name+"' />");
                                var selector=$('select',thd);
                                if(soptions.dataInit !== undefined) { soptions.dataInit(selector); }

                                //event
                                //TODO:parameter
                                selector.bind('onSelected',function(e,d){
                                    if(d){
                                        $(e.currentTarget).attr("selectedValue",d[$(e.currentTarget).ibselect("option","valueField")]);
                                    }else{
                                        $(e.currentTarget).removeAttr("selectedValue");
                                    }
                                 
//                                    if(d==undefined){triggerToolbar();return;}
                                        if(soptions.callBack!=undefined && $.isFunction(soptions.callBack)){
                                             soptions.callBack(selector,d);

                                             //false: don't search
                                            if(soptions.isSearch!=undefined && !soptions.isSearch){
                                                return;
                                            }
                                        }
                                    if(d||$(e.currentTarget).parent().find("input").val()==$(e.currentTarget).ibselect("option","noneItemText")){
                                        triggerToolbar();
                                    }
                                });
                        break;
